        ...
        with open(os.path.join(output_dir, 'run.tsv'), 'w') as f:
            for qid, rels in results.items():
                ...
                for i, (doc_id, score) in enumerate(rels.items()):
                    line = ' '.join([qid, 'Q0', doc_id, str(i + 1), str(score), 'rerank']) + '\n'
                    f.write(line)
    else:
        raise NotImplementedError
...
def run(
    ...
    retrieval_results: Dict[str, Dict[str, float]] = load_results(retrieval_result_path, retrieval_result_format)
    corpus_topk = {doc_id: corpus[doc_id] for _, rels in retrieval_results.items() for doc_id in rels}

    query_encoder_builder = encoder_builders.get_builder(encoder_name, ckpt_name, 'query')
    document_encoder_builder = encoder_builders.get_builder(encoder_name, ckpt_name, 'document')
    ...
    for b in tqdm(range(0, len(doc_ids), batch_size)):
        ...
        term_weight_dicts: List[Dict[str, float]] = document_encoder.encode(batch)  # no quantization here
