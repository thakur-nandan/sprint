...
def get_stop_ids(tok: PreTrainedTokenizer):
    ...
    stop_words.difference_update(
        ["where", "how", "what", "when", "which", "why", "who"]
    )
...
class TILDEv2(PreTrainedModel):
    ...
    def forward(self, qry_in, doc_in):
        ...
        qry_reps = torch.ones_like(
            qry_input_ids, dtype=torch.float32, device=doc_reps.device
        ).unsqueeze(2)
    ...
    def compute_tok_score_cart(
        ...
        scores, _ = (scores_no_masking * exact_match).max(
            dim=3
        )  # Q * LQ * D, max pooling
