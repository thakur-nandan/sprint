...
def cleanD(s, join=True):
    ...
    s = [
        (
            w
            if "." not in w
            else (" . " if len(max(w.split("."), key=len)) > 1 else "").join(
                w.split(".")
            )
        )
        for w in s
...
def cleanQ(s, join=True):
    ...
    s = [
        (
            w
            if "." not in w
            else (" " if len(max(w.split("."), key=len)) > 1 else "").join(w.split("."))
        )
        for w in s
    ]
    s = " ".join(s).split()
    s = [
        (
            w
            if "-" not in w
            else (" " if len(min(w.split("-"), key=len)) > 1 else "").join(w.split("-"))
        )
        for w in s
    ]
    s = " ".join(s).split()
...
class DeepImpact(PreTrainedModel):
    ...
    def convert_example(self, d, max_seq_length):
        ...
        inputs = self.tokenizer.encode_plus(
            d, add_special_tokens=True, max_length=max_length, truncation=True
        )
    ...
    def tokenize(self, q, d):
        ...
        match_indexes = list(
            set([doc_tokens.index(t) for t in query_tokens if t in doc_tokens])
        )
    ...
    def tokenize_documents(self, d):
        ...
        word_indexes = list(
            accumulate(
                [-1] + tokenized_content, lambda a, b: a + int(not b.startswith("##"))
            )
        )
    ...
    def forward(self, Q, D):
        ...
        for q, d in zip(Q, D):
            ...
            doc_partials.append(
                [(total_size + idx, tokens_split[i]) for idx, i in enumerate(term_idxs)]
            )
        ...
        input_ids = torch.tensor([f["input_ids"] for f in pairs], dtype=torch.long).to(
            self.device
        )
        attention_mask = torch.tensor(
            [f["attention_mask"] for f in pairs], dtype=torch.long
        ).to(self.device)
        token_type_ids = torch.tensor(
            [f["token_type_ids"] for f in pairs], dtype=torch.long
        ).to(self.device)
        ...
        def one(i):
            if len(X[i]) > 0:
                l = [
                    hidden_state[i, j] for j in X[i]
                ]  # + [mismatch_scores[i, j] for j in all_mismatches[i]]
        ...
        x = torch.arange(bsize).expand(len(pfx_sum), bsize) < torch.tensor(
            pfx_sum
        ).unsqueeze(1)
        y = torch.arange(bsize).expand(len(pfx_sum), bsize) >= torch.tensor(
            [0] + pfx_sum[:-1]
        ).unsqueeze(1)
        ...
        return (
            mask.type(torch.float32) @ y_score
        ), term_scores  # , ordered_terms #, num_exceeding_fifth
    ...
    def index(self, D, max_seq_length):
        ...
        pooled_output = torch.cat(
            [hidden_state[i, list(map(lambda x: x[1], X[i]))] for i in range(bsize)]
        )
...
class DeepImpactDocumentEncoder(DocumentEncoder):
    ...
    def encode(self, texts, **kwargs):
        ...
        max_seq_length = (
            max([len(tokenized_content) for tokenized_content, terms in super_batch])
            + 2
        )
