    ...
    for fpath in tqdm.tqdm(fpaths, disable=not show_progress_bar):
        ...
        with gzip.open(fpath, "r") as f:
            for line in f:
                ...
                if not check_range_only:
                    line_dict["vector"] = dict(
                        map(lambda x: (x[0], quantize(x[1])), term_dict.items())
                    )
...
def run(
    ...
    quantize_fn = build_quantize_fn(
        method, original_score_range, quantization_nbits, ndigits
    )
    ...
    for i, _pass in enumerate(passes):
        ...
        print(
            f"Pass {i+1}/{len(passes)} for quantization: check_range_only = {check_range_only}"
        )
        if i == 1:
            # Note here we replace `original_score_range` with `max_term_weight``
            quantize_fn = build_quantize_fn(
                method, max_term_weight, quantization_nbits, ndigits
            )

        fpaths = [
            os.path.join(collection_dir, fname) for fname in os.listdir(collection_dir)
        ]
        nprocs = min(len(fpaths), nprocs)
        nfpaths_per_proc = len(fpaths) // nprocs
        ranges = [
            (b, b + nfpaths_per_proc) for b in range(0, len(fpaths), nfpaths_per_proc)
        ]
        ranges[nprocs - 1] = (
            ranges[nprocs - 1][0],
            ranges[-1][1],
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--method", required=False, choices=["range-nbits", "ndigits-round"]
    )
    # for 'range-nbits':
    parser.add_argument(
        ...
        type=float,
        default=5,
        help="if set to -1, the score range will be computated automatically",
    )
