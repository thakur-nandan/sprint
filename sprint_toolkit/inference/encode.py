...
def one_process(
    ...
    for i, example in enumerate(
        tqdm.tqdm(data_iter, disable=not show_progress_bar, total=end - begin)
    ):
        if not (begin <= i < end):
            continue
        ...
        if (
            ...
            or len(batch_ids) + len(term_weights_chunk) >= chunk_size
            or i == end - 1
        ):
            with torch.no_grad():
            ...
            term_weights_batch = [
                {"id": doc_id, "contents": text, "vector": term_weights}
                for doc_id, text, term_weights in zip(
                    batch_ids, batch_texts, term_weights_batch
                )
            ]
...
def _run(
    ...
    chunk_size = min(
        chunk_size, range_size - 1
    )  # important for saving all the encoded data !!

    ranges = [(b, b + range_size) for b in range(0, nexamples, range_size)]
    ranges[len(gpus) - 1] = (
        ranges[len(gpus) - 1][0],
        ranges[-1][1],
    )  # merge all the leftovers into the last range
    ranges = ranges[: len(gpus)]  # keep only #GPUs ranges

    ranges[-1] = (
        ranges[-1][0],
        min(ranges[-1][1], nexamples),
    )  # make sure the last range is right, which is important for the criterion of ending point
    ...
    assert (
        nlines_encoded == nexamples
    ), f"Error: Did not encode all the documents ({nlines_encoded}/{nexamples})!!!"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--encoder_name",
        type=str,
        choices=["unicoil", "splade", "sparta", "tildev2", "deepimpact"],
        help="The name of the sparse encoder. For example, splade, unicoil, etc.",
    )
    parser.add_argument(
        "--ckpt_name", type=str, help="The name of the sparse model checkpoint."
    )
    parser.add_argument(
        "--data_name",
        type=str,
        help="The name of the dataset. For example, scifact, trec-covid, etc.",
    )
    parser.add_argument(
        "--data_dir",
        type=str,
        help="The directory where the BEIR data is present. For example, if you are using the scifact dataset, this is the directory where the corpus.jsonl is stored.",
    )
    parser.add_argument(
        "--gpus",
        nargs="+",
        type=int,
        help="The gpus to use. For example, --gpus 0 1 2 3",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        help="The output directory where the encoded data will be saved.",
    )
    parser.add_argument(
        "--batch_size",
        type=int,
        default=64,
        help="Default is 64. This is the number of documents to be encoded at a time. If you have a large GPU, you can increase this number to speed up the encoding process.",
    )
    parser.add_argument(
        "--chunk_size",
        type=int,
        default=100000,
        help="Default is 100k. This is the number of documents to be encoded at a time. If you have a large GPU, you can increase this number to speed up the encoding process.",
    )
