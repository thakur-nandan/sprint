...
class DeepImpact(nn.Module):
    def __init__(self, model_name, device, max_seq_length=300):
        ...
        hidden_dropout_prob = (
            ...
            if "hidden_dropout_prob" in dict(self.config)
    ...
    def tokenize(self, query, passage):
        ...
        a = [
            idx
            for i, idx in enumerate(match_indexes)
            if term_indexes[i] < self.max_seq_length
        ]
    ...
    def forward(self, queries, passages):
        ...
        for query, passage in zip(queries, passages):
            tokens, tokenized, term_idxs, token_idxs, seq_length = self.tokenize(
                query, passage
            )
